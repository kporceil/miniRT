...
/*   Updated: 2025/09/27 16:13:56 by kporceil         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static void	non_reflective_test(void **state)
{
	...
	assert_color_equal(color(0, 0, 0), reflected_color(*w, precompute(i, r), 1));
}
...
static void	reflective_test(void **state)
{
	...
	assert_color_equal(color(0.19032, 0.2379, 0.14274), reflected_color(*w, precompute((t_inter){w->objs + 2, sqrt(2)}, r), 1));
}
...
static void	color_reflective_test(void **state)
{
	...
	w->objs[2].material.reflective = 0.5;
	shape_set_matrix(w->objs + 2, matrix_translation(0, -1, 0));
	assert_color_equal(color(0.87677, 0.92436, 0.82918), shade_hit(*w, precompute((t_inter){w->objs + 2, sqrt(2)}, r), 1));
}
...
static void	limit_recursion_test(void **state)
{
	...
	w->objs[2].material.reflective = 0.5;
	shape_set_matrix(w->objs + 2, matrix_translation(0, -1, 0));
	assert_color_equal(color(0, 0, 0), reflected_color(*w, precompute((t_inter){w->objs + 2, sqrt(2)}, r), 0));
}
