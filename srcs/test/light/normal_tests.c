...
/*   Updated: 2025/11/26 18:17:52 by lcesbron         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_shape	s = sphere(0);
	t_tuple		n = normal_at(s, point(1, 0, 0), NULL);
...
{
	t_shape	s = sphere(0);
	t_tuple		n = normal_at(s, point(0, 1, 0), NULL);
...
{
	t_shape	s = sphere(0);
	t_tuple		n = normal_at(s, point(0, 0, 1), NULL);
...
{
	t_shape	s = sphere(0);
	t_tuple		n = normal_at(s, point(sqrt(3.0)/3.0, sqrt(3.0)/3.0, sqrt(3.0)/3.0), NULL);
...
static void	translated_sphere_normal_test(void **state)
{
	...
	t_tuple		n = normal_at(s, point(0, 1.70711, -0.70711), NULL);
...
static void	scaled_sphere_normal_test(void **state)
{
	...
	t_tuple		n = normal_at(s, point(0, sqrt(2.0/2.0), -sqrt(2.0/2.0)), NULL);
