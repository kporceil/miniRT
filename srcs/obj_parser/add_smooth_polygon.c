...
static void	fill_points_normals_tabs(t_tuple (*points)[3],
			t_tuple (*normals)[3], t_obj_parsing *p, t_vertice_data v[2])
{
...
void	add_smooth_polygon(char **args, t_vertice_data v1, t_obj_parsing *p,
						size_t len)
{
	t_vertice_data	v[2];
	t_tuple			points[3];
	...
	while (i < nb_vertices && i + 1 < len && !p->status)
	{
		...
		if (!v[0].vertice || !v[1].vertice || !v[0].normal || !v[1].normal)
			p->status = FILE_NEGATIVE_VERTICE;
		...
		if (group_add_shape(p->current_group,
				smooth_triangle(generate_uid(), points, normals)))
